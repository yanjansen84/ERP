...
import { Plus, Search, Network, X } from 'lucide-react';
...
const mockSubgrupos: Subgrupo[] = [
    { id: 1, name: 'Smartphones', parentGroup: 'Eletrônicos', productsCount: 15, status: 'Ativo' },
    { id: 2, name: 'Notebooks', parentGroup: 'Informática', productsCount: 10, status: 'Ativo' },
    { id: 3, name: 'Sofás', parentGroup: 'Móveis', productsCount: 5, status: 'Ativo' },
    { id: 4, name: 'Smartwatch', parentGroup: 'Eletrônicos', productsCount: 8, status: 'Ativo' },
    { id: 5, name: 'Mouses', parentGroup: 'Periféricos', productsCount: 20, status: 'Ativo' },
];

export default function Subgrupo() {
    const [subgrupos, setSubgrupos] = useState<Subgrupo[]>(mockSubgrupos);
    const [searchTerm, setSearchTerm] = useState('');
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [selectedSubgrupo, setSelectedSubgrupo] = useState<Subgrupo | null>(null);

    // Form State
    const [formData, setFormData] = useState({ name: '', parentGroup: '', status: 'Ativo' });

    const handleOpenModal = (subgrupo?: Subgrupo) => {
        if (subgrupo) {
            setSelectedSubgrupo(subgrupo);
            setFormData({ name: subgrupo.name, parentGroup: subgrupo.parentGroup, status: subgrupo.status });
        } else {
            setSelectedSubgrupo(null);
            setFormData({ name: '', parentGroup: '', status: 'Ativo' });
        }
        setIsModalOpen(true);
    };
    ...
    const columns: Array<{
        ...
        sortable?: boolean;
        width?: string;
        render?: (value: any, item: Subgrupo) => React.ReactNode;
    }> = [
            {
                key: 'id',
                title: 'Cód.',
            ...
            },
            {
                ...
                width: '30%',
                render: (value: string) => (
                    <span className="text-slate-600 bg-slate-50 px-2 py-1 rounded-md text-sm border border-slate-100">{value}</span>
                )
            },
            {
                key: 'productsCount',
                ...
                width: '20%',
                render: (value: number) => (
                    <span className="text-slate-500 text-sm">{value} itens</span>
                )
            },
            {
                key: 'status',
    ...
    const filteredSubgrupos = subgrupos.filter(s =>
        ...
        s.parentGroup.toLowerCase().includes(searchTerm.toLowerCase())
    );
    ...
    return (
        <div className="h-full flex flex-col p-6 animate-in fade-in duration-500">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800">Subgrupos</h1>
                    <p className="text-slate-500">Refine a categorização dos seus produtos.</p>
                </div>
                <button
                    ...
                    <div className="fixed inset-0 m-auto w-full max-w-md h-fit bg-white rounded-2xl shadow-2xl z-[70] p-6 animate-in zoom-in-95 duration-200">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-lg font-bold text-slate-800">
                                {selectedSubgrupo ? 'Editar Subgrupo' : 'Novo Subgrupo'}
                            </h2>
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Grupo Principal</label>
                                <select
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Nome do Subgrupo</label>
                                <input
                                    type="text"
                                    value={formData.name}
